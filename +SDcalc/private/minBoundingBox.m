function [bb, a]= minBoundingBox(X)
% compute the minimum bounding box of a set of 2D points
%   Use:   boundingBox = minBoundingBox(point_matrix). this algorithm is
%   based on the fact that: "For the convex polygon, a linear time
%   algorithm for the minimum-area enclosing rectangle is known. It is
%   based on the observation that a side of a minimum-area enclosing box
%   must be collinear with a side of the convex polygon" see wikipedia
...
k = convhull(X(1,:),X(2,:)); %k are indices
CH = X(:,k); % CH contains the coordinates of the convex hull
...
E = diff(CH,1,2);           % delta_x and delta_y for the edges. this will
                            % be used to calculate the angle relative to
                            % the x-axis
