...
%% Load block
tmp = load('N:\Project Z-stacks\New stacks\1K_0,25mg_30 nm_sample 2\TIF_1K_0.25 mg_30 nm_sample2\SegmentedStacks\PoreSize-ResultsAutomated-poreProps');
names = fieldnames(tmp);
assert(length(names)==1,'Unexpected, WTF')
Table05 = tmp.(names{1});
clear tmp names
% Load second file
tmp = load('N:\Project Z-stacks\New stacks\1K_0,5mg_30 nm_sample 2\tif_1k_0.5 mg_30 nm_Sample 2\SegmentedStacks\PoreSize-ResultsAutomated-poreProps');
names = fieldnames(tmp);
assert(length(names)==1,'Unexpected, WTF')
Table075 = tmp.(names{1});
clear tmp names
% Loqd third file
tmp = load('N:\Project Z-stacks\New stacks\1K_1 mg_30 nm_sample 2\tif_1K_1 mg_30 nm_Sample 2\SegmentedStacks\PoreSize-ResultsAutomated-poreProps');
names = fieldnames(tmp);
assert(length(names)==1,'Unexpected, WTF')
Table1 = tmp.(names{1});
clear tmp names
...
a.XScale = 'log';
a.YScale = 'log';
title({'CCDF-plot for area of pores','1K PIC hydrogel different concentrations'})
